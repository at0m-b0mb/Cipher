...
- **`cipher-social.png`** — the rendered 1280×640 raster, used as the README
  banner above and as the GitHub **social preview** (the image that appears when
  the repo is shared on X/Slack/Discord or surfaced in search and link unfurls).

**Turn it into the image people see when they search/share the repo:**

1. On GitHub, open the repo → **Settings** → scroll to **Social preview**.
2. Click **Edit** → **Upload an image** and choose `branding/cipher-social.png`.

That's the official mechanism — GitHub only uses a raster image uploaded here for
link previews (it can't read an SVG for that), which is why the PNG is pre-rendered
at the recommended 1280×640. To re-render after editing the SVG:
