...
enum Flashcards {
    ...
    static let deck: [Flashcard] = [
        ...
        // Expansion wave 3
        Flashcard("TLS Handshake", "The negotiation that opens an HTTPS connection: client and server agree a shared key and verify the server's certificate before any data flows. HTTPS is just HTTP carried inside TLS.", .fundamentals),
        Flashcard("Forward Secrecy", "Using an ephemeral Diffie-Hellman key per session so that stealing the server's long-term private key later can't decrypt traffic an attacker recorded earlier. Standard in TLS 1.3.", .fundamentals),
        Flashcard("Cipher Suite", "The bundle of algorithms a TLS connection agrees on — key exchange, the bulk cipher (e.g. AES-GCM) and the hash. TLS 1.3 trimmed the list to only modern, safe suites.", .fundamentals),
        Flashcard("SLAAC", "IPv6 Stateless Address Autoconfiguration — a host builds its own address from a router-advertised /64 prefix plus an interface id, no DHCP server required.", .networking),
        Flashcard("Anycast", "One IP address announced from many datacentres at once; internet routing delivers each user to the nearest. Powers CDNs, DNS roots and DDoS scrubbing.", .networking),
        Flashcard("Load Balancer", "A single front door that spreads incoming requests across a pool of identical backends, skipping any that fail a health check — the basis of horizontal scale and high availability.", .networking),
        Flashcard("CDN", "Content Delivery Network — a global mesh of edge servers caching content close to users to cut latency, absorb traffic spikes, and blunt DDoS by spreading load across the edges.", .networking),
        Flashcard("Credential Stuffing", "Replaying username:password pairs leaked from past breaches against other sites, betting that people reuse passwords. Defeated by MFA and breached-password screening.", .redTeam),
        Flashcard("NoSQL Injection", "Untrusted input smuggling query operators ($ne, $gt, $where) into a document-database query, changing its logic — from authentication bypass all the way to server-side code execution.", .redTeam),
        Flashcard("Operator Injection", "Supplying a query operator or nested object where the app expected a plain scalar, so the datastore executes the richer query the attacker assembled. The shape NoSQL injection takes.", .redTeam),
        Flashcard("Threat Modeling", "A structured, design-time analysis of what can go wrong in a system — done on a diagram before code exists, when fixes are cheapest. The purest form of 'shift left'.", .blueTeam),
        Flashcard("STRIDE", "Spoofing, Tampering, Repudiation, Information disclosure, Denial of service, Elevation of privilege — six threat prompts, each the mirror of a security property (auth, integrity, etc.).", .blueTeam),
        Flashcard("Trust Boundary", "A line in a design where the trust level changes (internet→server, user input→query). Threats concentrate where data crosses it, so it must be authenticated and validated.", .blueTeam),
        Flashcard("Purple Teaming", "A collaborative loop, not a team: red emulates a technique, blue checks detection, the gap is closed with a tested rule, and coverage is measured — improvement, not a pass/fail grade.", .blueTeam),
        Flashcard("Atomic Red Team", "A library of small, precise ATT&CK technique tests used to safely emulate adversary behaviour and validate that detections actually fire — a staple of purple teaming.", .blueTeam),

        // Expansion wave 4
        Flashcard("XOR", "The bitwise exclusive-or: output 1 only when bits differ. Self-inverse (x⊕k⊕k = x), which makes it the basis of stream ciphers and one-time pads.", .fundamentals),
        Flashcard("Bitwise Operators", "AND masks bits, OR sets them, XOR toggles/compares, NOT flips, and shifts (<< >>) multiply/divide by powers of two — the low-level toolkit behind crypto and exploitation.", .fundamentals),
        Flashcard("SQL", "Structured Query Language — the near-English language apps use to read and change data: SELECT, INSERT, UPDATE, DELETE, filtered with WHERE. Reading it is the prerequisite to understanding SQL injection.", .fundamentals),
        Flashcard("Prepared Statement", "A parameterized query that sends SQL structure and user data separately, so input is always bound as a value, never executed as code — the structural fix for SQL injection.", .fundamentals),
        Flashcard("Regular Expression", "A pattern describing the shape of text using character classes, quantifiers and anchors. Used by red teams to hunt secrets and by blue teams in detection rules — same skill, both sides.", .fundamentals),
        Flashcard("BGP", "Border Gateway Protocol — how Autonomous Systems announce which IP prefixes they can reach, building the internet's global routing. Historically trust-based, hence BGP hijacking and the RPKI fix.", .networking),
        Flashcard("Autonomous System", "An independently-administered network (ISP, cloud, enterprise) with its own AS number that exchanges routes with others via BGP. The internet is ~70,000 ASes cooperating.", .networking),
        Flashcard("BGP Hijacking", "Announcing a prefix you don't own so traffic is misrouted to you — used for interception and theft. RPKI route origin validation is the main defence.", .networking),
        Flashcard("SMTP / MX", "SMTP transfers mail between servers; the DNS MX record names which server receives a domain's mail. SMTP's lack of sender authentication is why spoofing — and SPF/DKIM/DMARC — exist.", .networking),
        Flashcard("QUIC / HTTP/3", "A UDP-based transport that folds TCP reliability and TLS 1.3 into one 1-RTT (0-RTT on resume) handshake, avoids head-of-line blocking, and survives network changes via a connection id.", .networking),
        Flashcard("SSRF", "Server-Side Request Forgery — coercing a server to fetch attacker-chosen URLs, reaching internal services, localhost panels and the cloud metadata endpoint (169.254.169.254) for IAM creds.", .redTeam),
        Flashcard("Cloud Metadata (IMDS)", "The 169.254.169.254 service that hands a VM its IAM role credentials. SSRF reaching it is a classic cloud-account-compromise pivot; IMDSv2's token requirement blunts it.", .redTeam),
        Flashcard("AD CS / ESC1", "Active Directory Certificate Services abuse: a template that lets low-priv users enroll, allows client auth, and lets the enrollee supply the subject — so they request a cert as Administrator and authenticate as Domain Admin.", .redTeam),
        Flashcard("Ransomware", "Malware that encrypts a victim's files for extortion, now usually with double extortion (stealing data to threaten a leak). Attackers delete backups first — so offline/immutable backups are the real defence.", .blueTeam),
        Flashcard("3-2-1 Backup Rule", "Keep 3 copies on 2 media with 1 offline/immutable. The offline copy is the one ransomware can't encrypt, making recovery — not payment — possible. Untested backups don't count.", .blueTeam),

        // Expansion wave 5
        Flashcard("Authentication vs Authorization", "Authentication proves who you are (login); authorization decides what you're allowed to do. Attacks target one or the other — stealing identity, or exceeding permissions.", .fundamentals),
        Flashcard("MFA Factors", "Something you KNOW (password), HAVE (phone/key), or ARE (biometric). True MFA combines factors from different categories, so one stolen factor isn't enough.", .fundamentals),
        Flashcard("TOTP", "Time-based One-Time Password — a 6-digit code both your app and the server derive from a shared seed + current time, changing every 30s. Proves possession without sending a reusable secret.", .fundamentals),
        Flashcard("Hypervisor", "The layer (VMware, KVM, Hyper-V) that runs virtual machines, each with its own full guest OS. Strong isolation because every VM has a separate kernel — heavier than containers.", .fundamentals),
        Flashcard("Container vs VM", "A VM ships a whole guest OS on a hypervisor (strong isolation, heavy); a container shares the host kernel via namespaces/cgroups (light, instant, thinner security boundary).", .fundamentals),
        Flashcard("Forward Proxy", "A proxy that fronts the clients — outbound traffic flows through it for filtering, logging, caching and anonymity (the destination sees the proxy's IP).", .networking),
        Flashcard("Reverse Proxy", "A proxy that fronts the servers — clients hit it and it forwards to a backend pool, doing TLS termination, caching, load balancing and WAF (nginx, HAProxy, CDNs).", .networking),
        Flashcard("WebSocket", "A persistent, full-duplex channel that starts as an HTTP Upgrade request and switches (101) to two-way frames — powering live chat, dashboards and games without polling.", .networking),
        Flashcard("CORS", "Cross-Origin Resource Sharing — response headers by which a server opts in to letting specific other origins read its responses, the controlled relaxation of the Same-Origin Policy.", .redTeam),
        Flashcard("CORS Misconfiguration", "Reflecting the request's Origin into Access-Control-Allow-Origin while allowing credentials lets ANY site — including the attacker's — read the victim's authenticated data.", .redTeam),
        Flashcard("Public Bucket Exposure", "A cloud storage bucket left world-readable: an anonymous LIST dumps every object and GET downloads them. No exploit — a customer-side misconfiguration, and the most common cloud breach.", .redTeam),
        Flashcard("SOAR", "Security Orchestration, Automation & Response — connects security tools and runs playbooks that auto-enrich, contain and ticket alerts, shrinking response time (MTTR) and analyst toil.", .blueTeam),
        Flashcard("Security Playbook", "A defined, often-automated response sequence triggered by an alert. Safe steps (enrichment) are automated; destructive ones (host isolation) are usually gated behind human approval.", .blueTeam),
        Flashcard("Secrets Management", "Keeping credentials out of code and in a vault (HashiCorp Vault, AWS Secrets Manager) that issues short-lived, rotatable, audited secrets — so a leak has a short fuse.", .blueTeam),
        Flashcard("Dynamic Secrets", "Unique, short-lived credentials generated on demand (e.g. a 15-minute DB user, then auto-revoked). A leaked dynamic secret expires almost immediately, shrinking the abuse window.", .blueTeam),

        // Expansion wave 6
        Flashcard("Compiler", "Translates human-readable source code into a CPU's binary machine instructions. Interpreted languages instead run via a runtime; either way, source becomes operations a chip can execute.", .fundamentals),
        Flashcard("Machine Code", "The raw binary instructions a CPU actually runs, fetched-decoded-executed one at a time. Reverse engineers disassemble it back into assembly to study a program with no source.", .fundamentals),
        Flashcard("CSPRNG", "A Cryptographically Secure PRNG — seeded from real OS entropy and built so its output can't be predicted or reversed. The only acceptable source for keys, tokens, nonces and salts.", .fundamentals),
        Flashcard("Entropy", "Genuine unpredictability the OS harvests from physical events (timing jitter, interrupts, input) to seed its cryptographic RNG. Predictable randomness breaks the crypto built on it.", .fundamentals),
        Flashcard("VLAN", "A Virtual LAN that splits one physical switch into multiple isolated broadcast domains, so groups share hardware without sharing a network — the basis of segmentation.", .networking),
        Flashcard("802.1Q Tag", "A 12-bit VLAN id stamped into a frame so switches keep VLAN traffic separated, especially across trunk links that carry several VLANs at once.", .networking),
        Flashcard("Network Segmentation", "Dividing a network into zones so a compromise in one can't reach the others — directly limiting lateral movement. VLANs do it at L2; microsegmentation pushes it to each workload.", .networking),
        Flashcard("BadUSB", "A malicious USB device that declares itself a trusted HID keyboard and injects a scripted payload at machine speed when plugged in. No file on disk, so file-based AV misses it.", .redTeam),
        Flashcard("HID Injection", "Keystroke injection via the Human Interface Device class — abusing the implicit trust every OS grants keyboards to 'type' attacker commands as the logged-in user.", .redTeam),
        Flashcard("DLL Hijacking", "Planting a malicious DLL of the expected name in a directory Windows searches before the real one (the app's own folder first), so a trusted process loads attacker code.", .redTeam),
        Flashcard("NIST CSF", "The NIST Cybersecurity Framework — organises a security programme into continuous functions: Identify, Protect, Detect, Respond, Recover (plus Govern in CSF 2.0).", .blueTeam),
